...
import { AccountStatusMapping, type RouterInput, type RouterOutput } from '@codeanker/api'
...
const columns = [
  ...
  column.display({
    ...
    header: 'Entscheidung',
    cell: ({ row }) => {
      if (row.original.account.status !== 'AKTIV') {
        return h('div', { class: 'text-danger-500 italic'}, `Account hat den Status ${AccountStatusMapping[row.original.account.status].human}`)
      }
...
const query: Query<AccessRequest> = (pagination, filter, orderBy) =>
  useQuery({
    queryKey: ['listAllGliederungAdminRequests', pagination, filter, orderBy, nochOffen, `mode=${mode?.mode}`],
    queryFn: () =>
      apiClient.access.listAllGliederungAdminRequests.query({
        ...
        filter: {
          accountId: mode?.mode === 'account' ? mode.accountId : undefined,
