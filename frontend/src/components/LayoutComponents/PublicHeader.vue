  ...
  <div class="flex items-center justify-between mb-4 py-6">
    ...
    <div class="text-sm text-right">
      <template v-if="loggedInAccount">
        <p>Du bist angemeldet als: <u>{{ loggedInAccount.person.firstname }} {{ loggedInAccount.person.lastname }}</u></p>
        <p><u>Account wechseln?</u> oder <u class="cursor-pointer" @click="() => logout(false)">Abmelden?</u></p>
      </template>
      <template v-else>
        <RouterLink
